...
In the following example, I show how to measure the root lenght of soybean seedlings. The images were gathered from Silva et al. 2019 ^[Silva LJ da, Medeiros AD de, Oliveira AMS (2019) SeedCalc, a new automated R software tool for germination and seedling length data processing. J Seed Sci 41:250–257. https://doi.org/10.1590/2317-1545V42N2217267].
...
To measure the root lenght, first we will ned to segment the root. Let's start choosing an index to segment the seedlings from background
...
In this case, the root lenght can be estimated by the `diam_max`, which is the maximum diameter of the object.
...
* `results`: a data frame that contains the measures for each individual object (in this case leaf) of each analyzed image.
...
## Area and solilidity for different species
...
Standard area diagrams (SAD) have long been used as a tool to aid the estimation of plant disease severity, serving as a standard reference template prior to or during the assessments.
...
Let's try to use the `NB` index to segment the leaf and background, and the `NGRDI` again to segment lesion and heathy tissues.
...
In the following example, the background have similar color than the midrib. In this case, cropping the image so that the background is removed would solve the issue.
